"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var mongoose_1 = require("mongoose");
// 2. Create a Schema corresponding to the document interface.
var userSchema = new mongoose_1.Schema({
    name: { type: String, required: true, unique: true },
    email: { type: String, required: true },
    dinero: { type: Number, required: true }
});
var productSchema = new mongoose_1.Schema({
    name: { type: String, required: true, unique: true },
    precio: { type: Number, required: true }
});
// 3. Create a Model.
var User = (0, mongoose_1.model)('User', userSchema);
var Product = (0, mongoose_1.model)('Producto', productSchema);
run()["catch"](function (err) { return console.log(err); });
function run() {
    return __awaiter(this, void 0, void 0, function () {
        var user1, user2, user3, product1, product2, product3, usuarios, productos, deleteUser, deleteProduct, usuarios2, productos2, user, prod, usuarios3, productos3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: 
                // 4. Connect to MongoDB
                return [4 /*yield*/, (0, mongoose_1.connect)('mongodb://127.0.0.1:27017/test')];
                case 1:
                    // 4. Connect to MongoDB
                    _a.sent();
                    user1 = new User({
                        name: 'Pau',
                        email: 'pau@garcia.com',
                        dinero: 0.01
                    });
                    return [4 /*yield*/, user1.save()];
                case 2:
                    _a.sent();
                    console.log("Se ha creado un nuevo usuario con mail -> " + user1.email); // 'pau@garcia.com'
                    user2 = new User({
                        name: "Rodrigo",
                        email: "rodrigo@gmail.com",
                        dinero: 0.02
                    });
                    return [4 /*yield*/, user2.save()];
                case 3:
                    _a.sent();
                    console.log("Se ha creado un nuevo usuario con mail -> " + user2.email); // 'rodrigo@gmail.com'
                    user3 = new User({
                        name: "Marta",
                        email: "marta@gmail.com",
                        dinero: 3
                    });
                    return [4 /*yield*/, user3.save()];
                case 4:
                    _a.sent();
                    console.log("Se ha creado un nuevo usuario con mail -> " + user3.email); //marta@gmail.com
                    product1 = new Product({
                        name: "Patatas",
                        precio: 3
                    });
                    return [4 /*yield*/, product1.save()];
                case 5:
                    _a.sent();
                    console.log("Se ha creado un nuevo producto con nombre -> " + product1.name);
                    product2 = new Product({
                        name: "Zanahorias",
                        precio: 100
                    });
                    return [4 /*yield*/, product2.save()];
                case 6:
                    _a.sent();
                    console.log("Se ha creado un nuevo producto con nombre -> " + product2.name);
                    product3 = new Product({
                        name: "Queso",
                        precio: 5
                    });
                    return [4 /*yield*/, product3.save()];
                case 7:
                    _a.sent();
                    console.log("Se ha creado un nuevo producto con nombre -> " + product3.name);
                    return [4 /*yield*/, User.find()];
                case 8:
                    usuarios = _a.sent();
                    console.log("Printeo todos los usuarios de mi BBDD");
                    console.log(usuarios);
                    return [4 /*yield*/, Product.find()];
                case 9:
                    productos = _a.sent();
                    console.log("Printeo todos los usuarios de mi BBDD");
                    console.log(productos);
                    return [4 /*yield*/, User.deleteOne({ name: "Rodrigo" })];
                case 10:
                    deleteUser = _a.sent();
                    console.log("Elimino a Rodrigo");
                    return [4 /*yield*/, Product.deleteOne({ name: "Queso" })];
                case 11:
                    deleteProduct = _a.sent();
                    console.log("Elimino el queso");
                    return [4 /*yield*/, User.find()];
                case 12:
                    usuarios2 = _a.sent();
                    console.log("Printeo los usuarios restantes de mi BBDD");
                    console.log(usuarios2);
                    return [4 /*yield*/, Product.find()];
                case 13:
                    productos2 = _a.sent();
                    console.log("Printeo los productos restantes de mi BBDD");
                    console.log(productos2);
                    return [4 /*yield*/, User.findOneAndUpdate({ name: 'Pau' }, {
                            name: 'Pablo'
                        }, { "new": true })];
                case 14:
                    user = _a.sent();
                    console.log("Actualizo a Pau");
                    console.log(user);
                    return [4 /*yield*/, Product.findOneAndUpdate({ name: 'Patatas' }, {
                            precio: 4
                        }, { "new": true })];
                case 15:
                    prod = _a.sent();
                    console.log("Actualizo el precio de la patata");
                    console.log(prod);
                    return [4 /*yield*/, User.find()];
                case 16:
                    usuarios3 = _a.sent();
                    console.log("Printeo la BBDD actualizada");
                    console.log(usuarios3);
                    return [4 /*yield*/, Product.find()];
                case 17:
                    productos3 = _a.sent();
                    console.log("Printeo la BBDD actualizada");
                    console.log(productos3);
                    return [2 /*return*/];
            }
        });
    });
}
